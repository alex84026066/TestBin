...
class Rain():
    def __init__(self, stationNameList, pastHours, obsUrl='CWB'):
    ...
    def MakePdictCsv(self,simUrl,simStartTime ,simTimelen = 3,futureHoursMax=24):
        for i in range(simTimelen):
            simTime = simStartTime+ timedelta(hours=i, minutes=0)
            SimTimeFormat = simTime.strftime('%Y%m%d%H')
            _ = self.simRainDict(simUrl,SimTimeFormat, futureHoursMax)
        
    def simRainDict(self, simUrl,SimTimeFormat, futureHoursMax=24):
        simRainDict = {}
        try:
            data = urlopen(os.path.join(_url[simUrl], SimTimeFormat)).read().decode('utf-8')
        ...
        if simFlag:
            ...
            for num, dataTimeApi in enumerate(simApiFormat):
                data = urlopen(os.path.join(
                    _url[simUrl], SimTimeFormat, zipName.format(num + 1, '.zip')))
                ...
                if num == 0:
                    fileName = str(int(SimTimeFormat)+1) + '.csv'
