...
class Timer:
    def __init__(self,simTime = None):
        '''
        type of nowtime should be datetime.
        '''
        rawTime = datetime.now()
        if simTime is not None:
            self.simTime = simTime
        else:
            self.simTime = rawTime
            
        self.nowTime = rawTime.replace(second=0, microsecond=0) - timedelta(minutes=10 + rawTime.minute % 10)
        self.initial()
    ...
    def simulate(self, futureHours):
        self.endTime = self.simTime + timedelta(hours=futureHours)
        ...
        self.simDateRange = date_range(self.simTime, self.endTime, freq='H')[1:]
